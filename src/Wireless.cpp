...
{
  printf("/**********BLE Test**********/\r\n"); 
  BLEDevice::init("ESP32");  // Initialize the BLE device
  BLEScan* pBLEScan = BLEDevice::getScan();
  pBLEScan->setActiveScan(true);  // Set active scanning

  // BLEScanResults is an object, not a pointer, so remove the pointer (*) from the declaration
  BLEScanResults foundDevices = pBLEScan->start(5);  // Start scan for 5 seconds

  int count = foundDevices.getCount();  // Get the number of found devices
  if (count == 0)
  {
    printf("No Bluetooth devices were scanned\r\n");
  }
  else
  {
  ...
  pBLEScan->clearResults();  // Clear the scan results
  BLEDevice::deinit(true);  // De-initialize BLE device
  vTaskDelay(100);  // Small delay for stability
  printf("/**********BLE Test Over**********/\r\n\r\n");

  return count;  // Return the number of scanned devices
}
